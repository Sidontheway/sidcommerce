...
export default function AlertButton(props) {
  return (
    <div
      role="alert"
      className="flex items-start md:items-center max-w-[600px] shadow-md bg-positive-100 pr-2 pl-4 ring-1 ring-positive-200 typography-text-sm md:typography-text-base py-1 rounded-md fixed top-10 right-10  "
    >
      <SfIconCheckCircle className="my-2 mr-2 text-positive-700 shrink-0" />
      <p className="py-2 mr-2"><span className='font-semibold'>{props.itemName}</span> has been added to the cart.</p>
      <button
        type="button"
